...
public class Shape {
	...
	boolean betterOverlaps(Shape t) {
		// TODO: Implement me.
		//if (tree.getBox().overlaps(t.tree.getBox())){
			//return true;
		//}
		/*if (this.getAbsBBox().overlaps(t.getAbsBBox())){
			for (Block a : blocks) {
				for (Block b : t.blocks) {
					if (Block.overlaps(a, d, b, t.d)){
						return true;
		...
		return false;*/
		return (this.tree.overlaps(d, t.tree, t.d));
	}
	...
	private boolean betterContains(Vector2D p) {
		...
		// Account for displacement of shape.
		Vector2D newP = p.minus(d);
		/*if (this.getAbsBBox().contains(newP)){
			for (Block b : blocks) {
				if (b.contains(newP))
					return true;
			}
		...
		return false;*/
		return this.tree.contains(newP);
	}
